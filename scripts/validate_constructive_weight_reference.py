#!/usr/bin/env python3
"""Validate the 2026 constructive-weight calculation and selection package."""
...
from rules.constructive_weight_reference import (  # noqa: E402
    APPROVAL_EVIDENCE_REQUIREMENT_ID,
    CALCULATION_RULE_ID,
    PROVENANCE,
    RULE_IDS,
    RULE_PACKAGE_ID,
    SELECTION_RULE_ID,
    TICKET_STATUS_EVIDENCE_REQUIREMENT_ID,
    VOLUME_EVIDENCE_REQUIREMENT_ID,
    RuleInputError,
    determine_constructive_weight_reference,
)
from rules.scale_reweigh_lower_reference import (  # noqa: E402
    RuleInputError as ScaleReferenceInputError,
...
FIXTURE_PATH = ROOT / "tests" / "fixtures" / "constructive-weight-reference" / "constructive-weight-cases.json"
...
def apply_record_mutations(records: dict, mutations: object, case_id: str) -> None:
    ...
    for mutation in mutations:
        ...
        if operation == "set":
            ...
            require(field in record, f"{case_id} cannot remove missing field {field}")
            del record[field]
        else:
            raise ValidationError(f"{case_id} unsupported record mutation operation {operation!r}")
...
def validate_registry_contract() -> None:
    ...
    require(RULE_PACKAGE_ID in packages, "constructive-weight package is missing")
    require(packages[RULE_PACKAGE_ID]["publication_status"] == "published", "constructive-weight package is not published")
    for rule_id in RULE_IDS:
        require(rule_id in rules, f"constructive-weight rule is missing: {rule_id}")
    ...
    require(
        deps_by_rule[CALCULATION_RULE_ID]
        == {"verified_shipment_volume", "constructive_weight_eligibility_reason", "responsible_ppso_approval"},
        "constructive calculation dependencies are incomplete",
    )
    require(
        deps_by_rule[SELECTION_RULE_ID]
        == {"final_constructive_weight_result", "valid_ticket_availability_status", "final_valid_ticket_weight_result"},
        "constructive selection dependencies are incomplete",
    )
    ...
    require(
        ...
        "constructive-weight evidence requirements are incomplete",
    )
    for rule_id in RULE_IDS:
        require(
            {record["source_claim_id"] for record in sources if record["rule_id"] == rule_id}
            == {"CLM-0025", "CLM-0033"},
            f"{rule_id} source scope is incomplete",
        )
    for reference in PROVENANCE:
        claim_id = reference["source_claim_id"]
        locator_id = reference["source_locator_id"]
        require(claims[claim_id]["source_locator_id"] == locator_id, f"claim {claim_id} locator mismatch")
        require(claims[claim_id]["interpretation_status"] == "reviewed", f"claim {claim_id} is not reviewed")
        require(locators[locator_id]["source_version_id"] == reference["source_version_id"], f"locator {locator_id} source mismatch")
...
def validate_result(result: dict, expected: dict, case_id: str) -> None:
    ...
    require(evidence.get("volume_requirement_id") == VOLUME_EVIDENCE_REQUIREMENT_ID, f"{case_id} volume evidence mismatch")
    require(evidence.get("approval_requirement_id") == APPROVAL_EVIDENCE_REQUIREMENT_ID, f"{case_id} approval evidence mismatch")
    require(evidence.get("ticket_status_requirement_id") == TICKET_STATUS_EVIDENCE_REQUIREMENT_ID, f"{case_id} ticket-status evidence mismatch")

    forbidden_fragments = ("fee", "tolerance", "refund", "billing", "item_code", "amount", "currency")
    ...
    if result["status"] == "FINAL":
        ...
        require(calculation.get("result") == expected.get("constructive_weight"), f"{case_id} constructive result mismatch")
        require(calculation.get("result_unit") == "lb", f"{case_id} constructive unit mismatch")
        require(calculation.get("rounding_rule") == "NONE_SOURCE_DOES_NOT_SPECIFY_ROUNDING", f"{case_id} rounding rule mismatch")
        require(selection.get("constructive_weight") == expected.get("constructive_weight"), f"{case_id} constructive candidate mismatch")
        require(selection.get("selected_weight") == expected.get("selected_weight"), f"{case_id} selected weight mismatch")
        require(selection.get("selected_source") == expected.get("selected_source"), f"{case_id} selected source mismatch")
        require(selection.get("weight_unit") == "lb", f"{case_id} selected unit mismatch")
        if expected.get("valid_ticket_weight_absent"):
            require("valid_ticket_weight" not in selection, f"{case_id} invented an unavailable ticket weight")
        else:
            require(selection.get("valid_ticket_weight") == expected.get("valid_ticket_weight"), f"{case_id} valid ticket weight mismatch")
    else:
...
def main() -> int:
    try:
        ...
        require(suite.get("fixture_set") == "SYNTHETIC_CONSTRUCTIVE_WEIGHT_REFERENCE_CASES", "fixture set is not labeled synthetic")
        paths = {
            key: (ROOT / suite[key]).resolve()
            for key in (
                "constructive_facts_fixture_path",
                "initial_weight_fixture_path",
                "reweigh_history_fixture_path",
            )
        }
        require(all(path.is_relative_to(ROOT) for path in paths.values()), "fixture path escapes repository")
        constructive_fixture = load_json(paths["constructive_facts_fixture_path"])
        initial_base = load_json(paths["initial_weight_fixture_path"]).get("base_case")
        reweigh_fixture = load_json(paths["reweigh_history_fixture_path"])
        require(constructive_fixture.get("data_status") == "SYNTHETIC", "constructive facts must be synthetic")
        ...
        require(isinstance(cases, list) and cases, "constructive-weight suite requires cases")
        for fixture in cases:
            ...
            reweigh_records = copy.deepcopy(reweigh_fixture["records"])
            ...
            ticket_result = select_scale_reweigh_lower_reference(
                {
                    "case_id": f"{case_id}-TICKET-REFERENCE",
                    "data_status": "synthetic",
                    "initial_weight_result": initial_result,
                    "completed_reweigh_result": reweigh_result,
                }
            )
            apply_path_mutations(ticket_result, fixture.get("valid_ticket_result_mutations", []), case_id)

            constructive_records = copy.deepcopy(constructive_fixture["records"])
            constructive_records["constructive_weight_assessments"][0]["ticket_weight_result_ref"] = ticket_result["case_id"]
            apply_record_mutations(constructive_records, fixture.get("constructive_record_mutations", []), case_id)
            supply_ticket = fixture.get("supply_ticket_result", True)
            candidate = {
                "case_id": case_id,
                "data_status": "synthetic",
                "records": constructive_records,
                "valid_ticket_weight_result": ticket_result if supply_ticket else None,
            }
            ...
            try:
                result = determine_constructive_weight_reference(candidate)
        ...
        print(f"PASS all {len(cases)} constructive-weight reference cases")
        return 0
    except (
        OSError,
